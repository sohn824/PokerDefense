...
이전 시트를 스타일 레퍼런스로 물린다. **구조가 있는 다섯을 먼저** 만든다 — 스카우트(`1`)·건슬링어(`2`)를 마쳤으니 남은 것은 `2+2` / `3` / `3+2` / `4`다.

| 우선 | 에셋 | 이름 | 구조 | 역할 | 성별 |
|---|---|---|---|---|---|
| — | `Unit_Scout` | 하이카드 스카우트 | 카드 1장, 권총 1정 | Rapid · **완료** | 남 |
| — | `Unit_Guard` | 원페어 건슬링어 | **2** 쌍권총 | Rapid · **완료** | 여 |
| 1 | `Unit_Ranger` | 투페어 트윈레인저 | **2+2** 양팔 2연장 | Multi | 여 |
| 1 | `Unit_Lancer` | 트리플 마크스맨 | **3** 3총구 | Heavy | 남 |
...
> **건슬링어로 개수 축을 확인했다.** 구조 숫자 **2**가 스카우트의 **1**과 대비되는지가 관건이었는데, 124px에서 갈리는 것은 팔 벌림 폭보다 **색 블록**이었다 — 검은 머리 + 흰 바지 대 금발 트윈테일 + 붉은 스커트 + 검은 타이츠다. 정면 종횡비는 0.485 대 스카우트 0.460으로 거의 같아, **가로로 넓음 축 하나만 믿으면 안 된다.** 남은 11종은 색·머리·복장 실루엣도 함께 갈라야 한다.
...
- **무기가 둘인 유닛은 `muzzleOffsetSecond`도 채운다** (DESIGN §10.4). 측면 스프라이트에서 **아래쪽 총**을 재서 넣는다. 비워 두면 단발로 그려지므로 무기가 하나인 유닛은 손대지 않는다.
  건슬링어 실측은 주 `(0.25, 0.26)` / 보조 `(0.19, 0.11)`이다. **트윈레인저·배터리가 같은 처리를 받는다.**
  > 정면·후면은 주 총구의 x를 뒤집어 쓰므로 따로 재지 않는다. 건슬링어 기준 근사 오차가 정면 9px / 후면 10px인데, 화염 스프라이트가 58px이라 총구를 덮는다. **이보다 무기가 더 벌어지는 유닛이 나오면 이 근사부터 다시 볼 것.**
...
| **쌍무기 총구 교대** | `UnitDefinition`·`CombatContext`·`UnitSlotView`·`BoardScreen` | **완료.** 발사 홀짝을 `CombatContext.ShotCountOf`가 센다. 무기가 하나인 유닛은 두 번째 좌표가 비어 단발로 떨어진다 |
...
4. 네 장을 **공통 캔버스에 가로 중앙·아래 정렬**로 다시 채운다 — 크기와 접지선이 같아야 방향이 바뀔 때 캐릭터가 위아래로 튀지 않는다
5. 임포트: `Sprite / Single`, **`npotScale = None`**, `PPU = 스프라이트 높이`, `Bilinear`, 밉맵 켬, 피벗 `BottomCenter`, `Alpha Is Transparency` 켬
...
6. `UnitDefinition`의 아트 4칸에 물린다
7. **총구 위치를 잰다.** 좌·우 스프라이트에서 가장 바깥으로 뻗은 픽셀이 총구 끝이다. 픽셀 좌표를 슬롯 로컬로 바꿔 `muzzleOffset`에 넣는다. **무기가 둘이면 아래쪽 총도 재서 `muzzleOffsetSecond`에 넣는다**
   ...
   > **재는 식이 맞는지는 스카우트로 확인할 수 있다.** 같은 식을 스카우트 스프라이트에 돌리면 기록된 값(우향 +0.206/+0.261, 좌향 −0.231/+0.257)이 그대로 나와야 한다.

8. **124px로 줄여 기존 유닛과 나란히 놓고 판별을 확인한다.** 종횡비만 보지 말 것 (§4-2단계)
9. 슬롯에 올려 방향 전환과 총구 화염 위치를 확인한다. **화염은 0.06초라 `Time.timeScale`을 낮춰야 보인다**

   > **족보가 뜰 때까지 기다리지 말 것.** Unity 메뉴 `PokerDefense > 족보 소환 (개발용)`에서 해당 족보를 눌러 바로 부른다. 손패의 92%가 하이카드·원페어라(DESIGN §3.3) 상위 족보는 기다려서는 사실상 못 본다.
