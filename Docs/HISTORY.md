...
- **진행 단계:** M0~M5 완료. M6(아트 교체 + 밸런싱) 착수 가능
...
- **루프:** 라운드가 자동으로 이어진다. 플레이어 입력이 필요한 지점은 **확정**과 **전투 시작** 둘뿐이다
- **다음 작업:** M6 — 아트 교체 + 밸런싱 데이터 채우기
- **대기 중인 결정:** DESIGN §7 열린 이슈 1번(저등급 편중 완화)이 **여전히 미결이며 이제 실제로 플레이해서 판단할 수 있다**. 2번(그리드 가득 참)은 "유닛 포기"로 임시 처리해 둔 상태. 4번(스테이지 길이)은 현재 3웨이브로 끝나며 재시작 수단이 없다
...
- **`GameFlowController`를 만들었다.** DESIGN §2에 이름만 있던 것이 이제 실체를 갖는다. 하는 일은 작다 — **자동으로 넘어가는 구간만** 맡고, 플레이어 입력이 필요한 구간은 각 컨트롤러가 그대로 한다.
  - 실제로 새로 붙은 자동 전이는 **`Result` → 다음 `Draw`** 하나뿐이다. 나머지는 이미 자동이었다.
  - 첫 라운드도 여기서 연다. `RoundController.Start()`의 자동 시작을 **없앴다** — 루프 주인이 둘이면 어긋난다.
- **`새 라운드` 버튼을 제거했다.** 루프가 자동이 되면 이 버튼은 어긋남만 만든다. 실제로 배치 도중에 누르면 대기 유닛이 조용히 사라지는 잠재 버그가 있었다.
- **`Combat` 진입 조건을 두 개로 막았다.** 루프가 연속으로 돌기 시작하니 드러난 구멍이다.
  - `round.Phase == RoundPhase.Place` — 교체 도중에 전투로 뛰면 그 라운드의 소환을 통째로 건너뛴다
  - `placement.Pending == null` — 배치 안 한 유닛을 남기고 넘어가면 다음 소환이 덮어써서 사라진다
  - 이 때문에 `RoundController.Phase`를 노출하고 `CombatController`에 `round` 참조를 추가했다
- **HUD는 라운드 기준으로 바꿨다.** 라운드 N이 웨이브 N을 치르므로 1:1이고, 플레이어가 겪는 단위는 라운드다. 게임 오버·스테이지 클리어도 같은 자리에 표시한다.
- **검증:** EditMode **115/115 통과**. Game 씬에서 라운드 3회를 실제로 연속 진행 —
  - 라운드 1: 유닛 1기로 시간 초과 → **라이프 20 → 16**, 자동으로 라운드 2 (새 손패 배분됨)
  - 라운드 2: 클리어(29.7초, 12마리) → 자동으로 라운드 3
  - 라운드 3: 배치가 **머지로 이어져 Guard ★2** 생성, 클리어(29.7초) → 전 웨이브 클리어
  - 종료 후 `IsFinished=True`, 버튼 3개 전부 비활성으로 잠김, HUD "스테이지 클리어"
  - 보드 유닛은 라운드 간 유지됨(누적 배치). **콘솔 에러·경고 0건**
- **남은 것:** 스테이지가 3웨이브로 끝나고 **재시작 수단이 없다**(§7 열린 이슈 4번). 라운드 전환에 연출·간격이 없어 전투 결과를 읽기 전에 다음 손패가 나온다.
