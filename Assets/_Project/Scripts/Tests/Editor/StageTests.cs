...
namespace PokerDefense.Tests
{
    ...
    public class StageTests
    {
        ...
        [Test]
        public void 보유_카드는_처음에_비어_있다()
        {
            Assert.AreEqual(0, StageWithCapacity(3).HeldCards.Count);
        }

        [Test]
        public void 산_카드는_보유_목록에_들어간다()
        {
            var stage = StageWithCapacity(3);
            var card = new Card(Rank.Ace, Suit.Spade);

            Assert.IsTrue(stage.TryAddHeldCard(card));
            CollectionAssert.Contains(stage.HeldCards, card);
        }

        [Test]
        public void 상한을_넘으면_더_담기지_않는다()
        {
            var stage = StageWithCapacity(2);

            Assert.IsTrue(stage.TryAddHeldCard(new Card(Rank.Two, Suit.Spade)));
            Assert.IsTrue(stage.TryAddHeldCard(new Card(Rank.Three, Suit.Spade)));
            Assert.IsFalse(stage.CanHoldMoreCards);

            var overflow = new Card(Rank.Four, Suit.Spade);
            Assert.IsFalse(stage.TryAddHeldCard(overflow));
            Assert.AreEqual(2, stage.HeldCards.Count);
            CollectionAssert.DoesNotContain(stage.HeldCards, overflow);
        }

        [Test]
        public void 손패에_놓으면_보유_목록에서_빠지고_다시_담을_수_있다()
        {
            var stage = StageWithCapacity(1);
            var card = new Card(Rank.King, Suit.Heart);

            stage.TryAddHeldCard(card);
            Assert.IsFalse(stage.CanHoldMoreCards);

            Assert.IsTrue(stage.TryRemoveHeldCard(card));
            Assert.AreEqual(0, stage.HeldCards.Count);
            Assert.IsTrue(stage.CanHoldMoreCards, "한 장 뺐으니 다시 담을 자리가 있다");
        }

        [Test]
        public void 안_가진_카드를_빼려_하면_실패한다()
        {
            var stage = StageWithCapacity(3);

            Assert.IsFalse(stage.TryRemoveHeldCard(new Card(Rank.Ace, Suit.Club)));
        }
    }
}
