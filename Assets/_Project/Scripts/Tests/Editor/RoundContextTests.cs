...
namespace PokerDefense.Tests
{
    public class RoundContextTests
    {
        ...
        [Test]
        public void 상점_카드를_놓으면_그_자리에_그대로_들어간다()
        {
            var round = Drawn();

            round.PlaceHeldCard(2, ShopCard);

            Assert.AreEqual(ShopCard, round.Hand[2]);
        }

        [Test]
        public void 상점_카드를_놓은_자리는_잠기지만_교체_장수에는_안_잡힌다()
        {
            var round = Drawn();

            round.PlaceHeldCard(2, ShopCard);

            Assert.IsTrue(round.IsLocked(2));
            Assert.AreEqual(4, round.ExchangeableCount);
            Assert.AreEqual(0, round.UsedExchanges, "상점 카드 배치가 교체로 잡혔다");
        }

        [Test]
        public void 상점_카드를_놓은_자리는_교체할_수_없다()
        {
            var round = Drawn();
            round.PlaceHeldCard(2, ShopCard);
        ...
        [Test]
        public void 교체한_자리에는_상점_카드를_놓을_수_없다()
        {
            var round = Drawn();
            round.Exchange(new[] { 2 });

            Assert.Throws<InvalidOperationException>(() => round.PlaceHeldCard(2, ShopCard));
        }

        [Test]
        public void 상점_카드를_놓아도_다른_자리_교체는_정상이다()
        {
            var round = Drawn();

            round.PlaceHeldCard(0, ShopCard);
            round.Exchange(new[] { 1, 2 });

            Assert.AreEqual(ShopCard, round.Hand[0]);
            Assert.AreEqual(2, round.UsedExchanges);
            Assert.AreEqual(2, round.ExchangeableCount);
        }

        [Test]
        public void 드로우_전에는_상점_카드를_놓을_수_없다()
        {
            var round = new RoundContext(1);

            Assert.Throws<InvalidOperationException>(() => round.PlaceHeldCard(0, ShopCard));
        }

        [Test]
        public void 교체를_끝낸_뒤에는_상점_카드를_놓을_수_없다()
        {
            var round = Drawn();
            round.FinishExchange();
        ...
        [TestCase(-1)]
        [TestCase(5)]
        public void 범위를_벗어난_자리에는_상점_카드를_놓을_수_없다(int index)
        {
            var round = Drawn();

            Assert.Throws<ArgumentOutOfRangeException>(() => round.PlaceHeldCard(index, ShopCard));
        }

        [Test]
        public void 보유_카드는_그_라운드_손패에_나오지_않는다()
        {
            var held = new[]
            {
            ...
            {
                var round = new RoundContext(seed, held);
                round.Draw();

                var seen = new List<Card>(round.Hand);
                round.Exchange(new[] { 0, 1, 2, 3, 4 });
                seen.AddRange(round.Hand);

                foreach (var card in held)
                {
                    Assert.IsFalse(seen.Contains(card), $"시드 {seed}에서 보유 카드 {card}가 손패에 나왔다");
                }
            }
        }
    }
