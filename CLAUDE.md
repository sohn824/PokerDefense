...
이 파일은 Claude Code가 이 저장소에서 작업할 때 자동으로 참조하는 지침이다.

## 프로젝트 진입점 (2026-09-13 갱신)

- 먼저 [Docs/HANDOFF.md](Docs/HANDOFF.md)를 읽는다. 현재 규칙, 코드 지도, 검증 한계와 다음 작업을 정리했다.
- 설계 기준은 [Docs/DESIGN.md](Docs/DESIGN.md), 남은 작업은 [Docs/POLISH.md](Docs/POLISH.md), 이력·반복 함정은 [Docs/HISTORY.md](Docs/HISTORY.md)다.
- 현재 핵심은 **손패를 직접 맞추는 플레이 + 매 손패 선택 교체 1회**다. Chip·상점·보관·랜덤 소환은 삭제했다. 이전 경제 기획을 복원하지 않는다.
- 구조 개편은 구현됐지만 현행 난도는 아직 사람 정상 런으로 검증하지 않았다. 런 저장/이어하기도 미구현이다.
- 과거 기획은 `Docs/LEGACY_LOOP_PLAN.md`에 분리했다. 현행 구현 명세로 사용하지 않는다. 작업 전 미커밋 diff를 확인한다.

## 기본 지침

- **답변할 시 항상 존댓말을 사용할 것.** 단 `Docs/`의 문서와 코드 주석은 기존 해라체를 유지한다 (§3의 "기존 스타일을 따른다").
- **주력 언어는 C++/C#.**
- **git 커밋·푸시는 하지 않는다.** 작업을 마치면 변경사항을 작업 트리에 그대로 두고 무엇이 바뀌었는지만 보고한다. 커밋은 사용자가 직접 한다. 사용자가 그 작업에 대해 명시적으로 "커밋해줘"라고 지시한 경우에만 커밋한다.
...
- 사용자가 작성한 주석은 수정하지 않는다. 명확히 틀린 내용일 경우에만 검증을 요청한다.
